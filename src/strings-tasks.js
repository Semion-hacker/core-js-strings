...
function formatTime(minutes, seconds) {
  const min1 = String(minutes).padStart(2, '0');
  const sec1 = String(seconds).padStart(2, '0');
  return `${min1}:${sec1}`;
}
...
function isPalindrome(str) {
  const s1 = str.replace(/[^0-9a-z]/gi, '').toLowerCase();
  for (let i = 0; i < Math.floor(s1.length / 2); i += 1) {
...
function findLongestWord(sentence) {
  const sen1 = sentence.split(' ');
  let sum = '';
  for (let i = 0; i < sen1.length; i += 1) {
    const cur = sen1[i];
    if (cur.length > sum.length) {
      sum = cur;
    }
  }
